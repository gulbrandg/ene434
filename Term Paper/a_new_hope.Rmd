...
To create a price-model using exchange, load and vrep as explanatory variables, we need to synthesize future data for these variables. Additionally, the model must account for the maximum transmission capacity of each interconnector, ensuring that exchange cannot exceed the physical constraints of transmission.  
...
The plot above indicates a varying degree of positive correlation between exchange and variable renewable energy production in border areas, showing a trend of import when vrep is available. However, this correlation is negligible for the UK. Conversely, DK and DE appear to have a linear relationship between exchange and vrep. This might suggest that vrep production is a driver for exchange between NO and these areas. This does not seem to be the case for UK where there is no incline.  
...
vrep_inc <- (seq(1:N_DAYS) / N_DAYS * 0.5) + 1
...
create_dataset_area <- function(a) {
  ...
  old_df <-
    ...
    select(date, area, exchange, vrep, load) %>% 
    mutate(increasing_vrep = NA,
           increasing_load = NA)
  ...
  new_df <- 
    tibble(date = DATE_RANGE,
           area = a,
           exchange = NA,
           vrep = unlist(map2(a, month(date), draw_vrep)),
           increasing_vrep = unlist(map2(a, month(date), draw_vrep)) * vrep_inc,
...
forecast_exchange <- function(a) {
  ...
  model <- 
    data_fit %>% 
    fill_gaps() %>%
    model(mod = ARIMA(exchange ~ vrep + load + fourier(period = 7, K = 3)))
  
  model %>% 
    gg_tsresiduals() %>% 
    print()
  
  # Scenario 1: steady vrep and steady load
  pred1 <- 
    model %>% 
  ...
  # Scenario 2: Increasing vrep and steady load
  pred2 <- 
    model %>% 
    forecast(., new_data = (data_pred %>% mutate(vrep = increasing_vrep))) %>% 
    mutate(f = unlist(map2(.mean, rep(import_cap, N_DAYS), min))) %>% 
    mutate(f = unlist(map2(f, rep(export_cap, N_DAYS), max)))

  # Scenario 3: Steady vrep and increasing load
  pred3 <- 
    model %>% 
  ...
  # Scenario 4: Increasing vrep and increasing load
  pred4 <- 
    model %>% 
    forecast(., new_data = (data_pred %>% mutate(vrep = increasing_vrep, load = increasing_load))) %>% 
    mutate(f = unlist(map2(.mean, rep(import_cap, N_DAYS), min))) %>% 
    mutate(f = unlist(map2(f, rep(export_cap, N_DAYS), max)))
...
```{r, warning=FALSE, message=FALSE}
# Scenario 1
print("Scenario 1: Steady vrep and steady load")
pred1 <- 
  model %>% 
...
# Scenario 2
print("Scenario 2: Increasing vrep and steady load")
pred2 <- 
  model %>% 
...
# Scenario 3
print("Scenario 3: Steady vrep and increasing load")
pred3 <- 
  model %>% 
...
# Scenario 4
print("Scenario 4: Increasing vrep and increasing load")
pred4 <- 
  model %>% 
...
tibble(Scenario = c("Scenario 1", "Scenario 2", "Scenario 3", "Scenario 4"),
  ...
  pander()
```
In the table above the calculated mean and standard deviation for each scenario are presented. We compare with Scenario 1 to examine how an increase in variable renewable production and an increase in consumption affects the spot price level to 2030. 

Scenario 2, with an 50% increase in variable renewable energy and steady consumption to 2030, has a 6.49% lower price level than the base scenario. 
