...
We also need to synthesize the future load.  
...
model <- 
  pdf %>% 
  fill_gaps() %>%
  model(mod = ARIMA(price ~ exchange + load + 
                      fourier(period = 7, K = 3)))
...
tibble(Scenario = c("Scenario 1", "Scenario 2", "Scenario 3", "Scenario 4"),
       Mean = c(mean(pred1$.mean), mean(pred2$.mean), mean(pred3$.mean), mean(pred4$.mean)),
       "% diff from Scenario 1" = 
         c(
           paste0(round(abs(mean(pred1$.mean) - mean(pred1$.mean)) / mean(pred1$.mean) * 100, 2), "%"), 
           paste0(round(abs(mean(pred1$.mean) - mean(pred2$.mean)) / mean(pred1$.mean) * 100, 2), "%"), 
           paste0(round(abs(mean(pred1$.mean) - mean(pred3$.mean)) / mean(pred1$.mean) * 100, 2), "%"), 
           paste0(round(abs(mean(pred1$.mean) - mean(pred4$.mean)) / mean(pred1$.mean) * 100, 2), "%")))
