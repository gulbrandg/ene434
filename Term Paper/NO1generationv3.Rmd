...
```{r}
test <- read_csv("Cross Border Flow/Cross-Border Physical Flow_201801010000-201901010000norden.csv")

cross_border_flow <- function(country, year) {
  read_csv(paste0("Cross Border Flow/", country, "_", year, ".csv")) %>% 
    .[,c(1,2,3)] %>% 
    ...
    group_by(date) %>% 
    summarize(import = sum(import, na.rm = TRUE),
              export = sum(export, na.rm = TRUE),
              net_export = sum(export, na.rm = TRUE) - sum(import, na.rm = TRUE),
